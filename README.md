...
`plasma-widgets/` has ten ornate-framed Plasma desktop widgets, styled to
...
wallust template needed. Every frame also casts a soft accent-colored glow
(`ctx.shadowColor`/`shadowBlur` on the Canvas), and the dash-style meter
bars fade smoothly between states instead of snapping.
...
- **Plasmalust Media** - MPRIS now-playing panel via `playerctl` (art,
  title/artist/album, progress, prev/play-pause/next - transport icons are
  Canvas-drawn vectors, not emoji glyphs, so they stay theme-colored).
...
- **Plasmalust Wallpaper Picker** - searchable thumbnail grid of
  ...
  called incrementally at the end of every `set-theme` run). Idles as a thin
  title-only line and expands to the full grid on hover - the widget's own
  reserved footprint can't shrink (see the Folder View note below), so only
  what's *painted* inside it changes.
- **Plasmalust Menu** - dropdown quick-launcher (`QQC2.Menu`, not a resized
  widget - sidesteps the Folder View limitation entirely): app launches,
  a toybox submenu, and lock/logout/reboot/shutdown.
- **Plasmalust Clock** - big digital time + date, pure QML/JS, no shell calls.
- **Plasmalust Calendar** - month grid, today highlighted, prev/next nav.
- **Plasmalust Weather** - current conditions via `wttr.in` (no API key).
- **Plasmalust Net/Disk** - live up/down network speed + disk read/write,
  computed from `/sys/class/net/*/statistics/` and `/proc/diskstats` deltas.

Install/upgrade all ten, plus the visualizer's cava bridge service and the
